...
<section class="rounded-lg bg-white p-5 shadow-sm ring-1 ring-slate-100">
    <form method="GET" class="mb-5 grid gap-3 lg:grid-cols-[1fr_12rem_auto] lg:items-end">
        <label class="text-xs font-extrabold uppercase text-slate-400">Search
            <input name="search" value="{{ request('search') }}" placeholder="Name or email" class="mt-1 h-10 w-full rounded-lg border-slate-200 text-sm">
        </label>
                ...
                <tr class="align-top">
                    <td class="py-4 pr-4">
                        <div class="flex items-center gap-3">
                            <div class="grid h-10 w-10 place-items-center rounded-lg bg-indigo-50 text-sm font-extrabold text-indigo-700">{{ strtoupper(substr($customer->parent_name, 0, 1)) }}</div>
